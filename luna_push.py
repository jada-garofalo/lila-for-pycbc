...
class luna_push:
    def __init__(self, lunar_det_lat=[-(np.pi/2)], lunar_det_lon=[0], lunar_det_h=0):
        self.msp = MoonSurfacePoint(lunar_det_lat, lunar_det_lon, lunar_det_h)
        self.apx = "TaylorF2"
    ...
    def set_mspole_location_eff(self, msp, obstimes):
        '''
    ...
    def time_delay_eff(self, observation_times, msp, ra, dec, frame='icrs', debug=False): 
        '''
        ...
        moon_positions_q = self.set_mspole_location_eff(msp, observation_times)
    ...
    def add_detector_on_moon(self, name, longitude, latitude,
    ...
    def luna_shift(self, f_lower_LILA, f_final_LILA, mass1, mass2, delta_t, ra, dec, 
        ...
        hp_LILA, hc_LILA = get_td_waveform(approximant=self.apx, mass1=mass1,
        ...
        _lunar_detectors = self.add_detector_on_moon("LILA", msp.lon, msp.lat)

        t_det = np.asarray(hp_LILA.sample_times)
        t0 = t_det[0]
        t_abs = start_time + (t_det - t0) * u.s
        delays = self.time_delay_eff(t_abs, msp, ra, dec, frame=frame, debug=debug)
    ...
    def antenna_pattern_alt(self, detector, ra, dec, polarization, obstime, s_obstime,
                            frequency=0, polarization_type='tensor'):
        '''
    ...
    def time_dependent_strain(self, detector, hp, hc, ra, dec, polarization, s_obstime):
        ...
        for t in hp.sample_times:
            ...
            fp_i, fc_i = self.antenna_pattern_alt(
                                         detector,
                                         ra=ra,
                                         dec=dec,
